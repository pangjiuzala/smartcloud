...
public class UserRegisterActivity extends Activity {
	...
	// Assist.transActivity(UserRegisterActivity.this,
	...
	Handler myhandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (msg.arg1 == 0) {
				...
				Toast.makeText(getApplicationContext(), "当前用户已存在!",
						Toast.LENGTH_SHORT).show();
				...
				Assist.transActivity(UserRegisterActivity.this,
						UserRegisterActivity.class);
				Toast.makeText(getApplicationContext(), "注册失败,请检查输入或网络设置",
						Toast.LENGTH_SHORT).show();
			}
...
//				WindowManager.LayoutParams.FLAG_FULLSCREEN);
		...
		MyActivityManager.addActivity(UserRegisterActivity.this);
				...
				, new OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						progressDialog = ProgressDialog.show(
								UserRegisterActivity.this, "正在注册", "请您稍后...",
								false, true);
								...
								try {
									...
									ResultSet rs = s
											...
											.executeQuery("select  *  from   tpl_login where username="
													...
													+ password + ";");
									if (!rs.next()) {
										s.executeUpdate("insert into tpl_login(username,password ) values ('"
												+ username
												+ "','"
												+ password
												+ "')");
