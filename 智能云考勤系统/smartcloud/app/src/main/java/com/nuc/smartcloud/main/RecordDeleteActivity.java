...
public class RecordDeleteActivity extends Activity {
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		if (keyCode == KeyEvent.KEYCODE_BACK) {
			...
			Assist.transActivity(RecordDeleteActivity.this,
	...
	Handler myhandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (msg.arg1 == 0) {
				...
				Toast.makeText(getApplicationContext(), "清空失败,请检查输入或网络设置",
						Toast.LENGTH_SHORT).show();
...
//				WindowManager.LayoutParams.FLAG_FULLSCREEN);
		...
		MyActivityManager.addActivity(RecordDeleteActivity.this);
		...
		// 设置对话框的标题
		builder.setTitle("请输入您要清空记录的员工信息");
		// 装载/res/layout/login.xml界面布局

		final TableLayout loginForm = (TableLayout) getLayoutInflater()
				.inflate(R.layout.activity_starff, null);
				...
				, new OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						progressDialog = ProgressDialog.show(
								RecordDeleteActivity.this, "清空中", "请您稍后...",
								false, true);
						new Thread(new Runnable() {
							...
							public void run() {
								...
								try {
									...
									EditText Name = (EditText) loginForm
											.findViewById(R.id.num);
									String nums = Name.getText().toString()
											.trim();
									Name.getText().clear();
									...
									if (rs.next()) {
										...
										s.executeUpdate("update   tpl_members  set member_exam_record='0' where member_id ="
												+ nums
												+ " or id="
												+ nums
												+ " or member_phone=" + nums);
										s.executeUpdate("update   tpl_members  set member_late='0'  where member_id ="
												+ nums
												+ " or id="
												+ nums
												+ " or member_phone=" + nums);
										s.executeUpdate("update   tpl_members  set member_cut_class='0' where member_id ="
												+ nums
												+ " or id="
												+ nums
												+ " or member_phone=" + nums);
										s.executeUpdate("update   tpl_members  set  member_ontime='0' where member_id ="
												+ nums
												+ " or id="
												+ nums
												+ " or member_phone=" + nums);
